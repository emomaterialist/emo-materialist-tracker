...
function buildScheduleRows() {
  ...
  DAYS_OF_WEEK.forEach(function(day, i) {
    ...
    row.innerHTML =
      ...
      '<input type="number" id="sched-times-' + i + '" min="0" max="20" value="1">' +
      '<label><input type="checkbox" id="sched-flex-' + i + '"> Flex</label>';
    el.appendChild(row);
  });
}
...
async function submitAddHabit() {
  ...
  var schedEntries = [];
  DAYS_OF_WEEK.forEach(function(_, i) {
    var times = parseInt(document.getElementById('sched-times-' + i).value) || 0;
    var flex = document.getElementById('sched-flex-' + i).checked;
    if (times > 0) schedEntries.push({ day_of_week: i, times_required: times, is_flexible: flex });
  });
  if (schedEntries.length === 0) return showMsg('add-habit-msg', 'Set at least one day with times > 0.', 'error');
  ...
  var schedRows = schedEntries.map(function(s) { return { habit_id: habitId, day_of_week: s.day_of_week, times_required: s.times_required, is_flexible: s.is_flexible }; });
...
async function saveLog(habitId, logDate, timesCompleted) {
  ...
  // Update local lookup too so re-renders are instant
