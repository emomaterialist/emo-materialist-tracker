  ...
  for(var dow=0;dow<7;dow++){
    ...
    for(var wk=0;wk<totalCols;wk++){
      ...
      if(dt.getFullYear()!==year){ cell2.style.background='transparent'; }
        ...
        (function(ds,v,tip){ cell2.addEventListener('mouseenter',function(){ tip.innerHTML='<strong>'+ds+'</strong><br>'+(v>0?v+' habit'+(v!==1?'s':'')+' completed':'No activity logged'); tip.style.display='block'; }); cell2.addEventListener('mousemove',function(e){ tip.style.left=(e.clientX+12)+'px'; tip.style.top=(e.clientY-40)+'px'; }); cell2.addEventListener('mouseleave',function(){ tip.style.display='none'; }); })(dateStr,val,tooltip);
      }
...
function renderRadar() {
  ...
  // Calculate actual completion % per category (done/required)
  var catDone={}, catRequired={}, catAllDone={}, catAllRequired={};
  RAW_HABITS.forEach(function(h){
    var hid=String(h.id), cat=h.category||'General';
    if(!catDone[cat]){catDone[cat]=0;catRequired[cat]=0;}
    if(!catAllDone[cat]){catAllDone[cat]=0;catAllRequired[cat]=0;}
    // This month
    for(var d=1;d<=DAYS_IN_MONTH;d++){
      var iso=currentYear+'-'+String(currentMonth).padStart(2,'0')+'-'+String(d).padStart(2,'0');
      var dow=(firstDayMonBased+d-1)%7;
      ...
      catRequired[cat]+=req;
      catDone[cat]+=(logsLookup[hid]&&logsLookup[hid][iso])||0;
    }
    // All time
    Object.keys(allTimeLogs[hid]||{}).forEach(function(iso){
      ...
      catAllDone[cat]+=(allTimeLogs[hid][iso]||0);
    });
  });
  var cats=Object.keys(catDone);
